...
| 过期清理 | actions/stale 处理等待状态，普通脚本提醒其他长期无活动内容；作者回复 / 提交后清除 PR 的等待作者状态 |
...
草稿 PR 不参与。标签政策互斥，避免对同一条内容重复提醒。
只使用一个 actions/stale 实例处理两种等待状态：上游状态缓存的 key 固定为 `_state`，多个实例会相互覆盖。
其他内容的 60 天提醒由无跨运行缓存的脚本处理，并更新同一条机器人提醒评论。
回复或更新会撤销 stale 状态；PR 作者回复或有新提交时，同时移除 awaiting-author。
需要作者继续处理时由维护者重新标记，不把“等待维护者 review”的 PR 自动关闭。
actions/stale 每次最多 100 次操作，普通提醒每次最多处理 20 条；后续日程继续处理剩余内容。
